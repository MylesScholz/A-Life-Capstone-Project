...
func _pressed():
	...
	if (Engine.time_scale > GlobalVariables.min_speed):
		Engine.time_scale /= 2
		get_parent().get_node("SpeedDisplay").call("update_speed_display", Engine.time_scale)
	...
	if (Engine.time_scale == 0 and (GlobalVariables.time_scale_backup_one > GlobalVariables.min_speed)):
		...
		get_parent().get_node("SpeedDisplay").call("update_speed_display", GlobalVariables.time_scale_backup_one)
